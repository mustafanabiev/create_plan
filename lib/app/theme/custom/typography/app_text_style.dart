import 'package:create_plan/app/app.dart';
import 'package:flutter/material.dart';
import 'package:google_fonts/google_fonts.dart';

class AppTextStyle {
  static TextStyle roboto = GoogleFonts.roboto(
    fontSize: 25,
    color: AppColors.textColor,
    fontWeight: FontWeight.w400,
  );

  static TextStyle metal = GoogleFonts.metal(
    fontSize: 25,
    color: AppColors.textColor,
    fontWeight: FontWeight.w400,
  );

  static TextStyle mandali = GoogleFonts.mandali(
    fontSize: 25,
    color: AppColors.textColor,
    fontWeight: FontWeight.w400,
  );

  static TextStyle openSans = GoogleFonts.openSans(
    fontSize: 25,
    color: AppColors.textColor,
    fontWeight: FontWeight.w400,
  );

  static TextStyle robotoText40w800 = GoogleFonts.roboto(
    fontSize: 40,
    fontWeight: FontWeight.w800,
  );
  static TextStyle robotoTextColor30w800 = GoogleFonts.roboto(
    fontSize: 30,
    color: AppColors.textColor,
    fontWeight: FontWeight.w800,
  );
  static TextStyle robotoTextColorBlack22w500 = GoogleFonts.roboto(
    fontSize: 22,
    color: AppColors.black,
    fontWeight: FontWeight.w500,
  );
  static TextStyle robotoTextColorWhite22w500 = GoogleFonts.roboto(
    fontSize: 22,
    color: AppColors.white,
    fontWeight: FontWeight.w500,
  );

  static TextStyle roboto16w500 = GoogleFonts.roboto(
    fontSize: 16,
    fontWeight: FontWeight.w500,
  );
  static TextStyle robotoTGrey16w500 = GoogleFonts.roboto(
    fontSize: 16,
    fontWeight: FontWeight.w500,
    color: const Color.fromARGB(255, 162, 156, 156),
  );
  static TextStyle roboto18w500 = GoogleFonts.roboto(
    fontSize: 18,
    fontWeight: FontWeight.w500,
  );
  static TextStyle robotoTGreyFromARGB18w500 = GoogleFonts.roboto(
    fontSize: 18,
    color: const Color.fromARGB(255, 138, 136, 131),
    fontWeight: FontWeight.w500,
  );
  static TextStyle robotoTGrey18w500 = GoogleFonts.roboto(
    fontSize: 16,
    fontWeight: FontWeight.w500,
    color: const Color.fromARGB(255, 162, 156, 156),
  );
}
